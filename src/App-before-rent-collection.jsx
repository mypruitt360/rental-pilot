...
export default function App(){
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 2.2 · Rent Collection</small><br/><button className="text-button" style={{marginTop:10}} onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
    </aside>
    <main>
      <header>
...
function CommandCenter({properties,residents,payments,expenses,maintenance,month,onRecord,onGo}){
  ...
  return <>
    ...
    <section className="stats">
      ...
      <Stat label="Net collected" value={money(netCollected)} sub="Collected less recorded expenses"/>
    ...
    <section className="grid two">
      <div className="card">
        ...
        <div className="ledger">
          ...
          {dueResidents.map(r=><div className="ledger-row" key={r.id}>
            <span>
              <strong>{r.name}</strong>
              <small>{r.daysLate>0?`${r.daysLate} day${r.daysLate===1?'':'s'} late`:r.paid>0?'Partial payment':'Due'}</small>
            </span>
