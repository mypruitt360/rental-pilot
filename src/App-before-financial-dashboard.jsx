...
export default function App(){
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 1.5 · Financial Dashboard</small><br/><button className="text-button" style={{marginTop:10}} onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
    </aside>
    <main>
      <header>
...
function PropertyDetail({property,residents,payments,expenses,maintenance,documents,month,onBack,onRecord,onEdit,onOpenDocument}){
  ...
  const potentialOperatingCashFlow=potentialRent-fixedMonthly-monthExpenses;
  const annualGrossAtCurrent=currentRent*12;
  const annualNOI=Math.max(0,annualGrossAtCurrent-fixedAnnual-(ytdExpenses*(12/Math.max(1,(new Date().getMonth()+1)))));
  ...
  const annualizedCashFlow=operatingCashFlow*12;
  ...
  return <>
    ...
    <section className="stats" style={{marginBottom:16}}>
      <Stat label="Actual cash flow" value={money(operatingCashFlow)} sub={`${monthLabel(month)} after fixed + other expenses`} tone={operatingCashFlow>=0?'':'opportunity'}/>
      <Stat label="YTD rent collected" value={money(ytdCollected)} sub={`${year} payments received`}/>
      <Stat label="YTD expenses" value={money(ytdExpenses)} sub={`${year} recorded expenses`}/>
      ...
      <Stat label="Cash-on-cash" value={pct(cashOnCash)} sub="Annualized current cash flow ÷ purchase basis"/>
    ...
    <section className="grid two">
      ...
      <div className="card">
        ...
        <div className="result-grid">
          ...
          <Result label="Other expenses" value={`${money(monthExpenses)}/mo`}/>
      ...
      <div className="card">
        ...
        <div className="result-grid">
          ...
          <Result label="Estimated annual NOI" value={money(annualNOI)}/>
