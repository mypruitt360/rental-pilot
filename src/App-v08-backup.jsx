...
export default function App(){
  ...
  const addMaintenance = item => saveMaintenance([{id:`maint-${Date.now()}`,status:'Open',createdAt:todayISO(),estimatedCost:Number(item.estimatedCost)||0,actualCost:Number(item.actualCost)||0,...item},...maintenance]);
  ...
  const addExpense = expense => saveExpenses([{id:`exp-${Date.now()}`,...expense,amount:Number(expense.amount)||0},...expenses]);
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 0.8 · Property Detail</small><br/><button className="text-button" style={{marginTop:10}} onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
    </aside>
    <main>
      <header>
...
function PropertyDetail({property,residents,payments,expenses,maintenance,documents,month,onBack,onRecord}){
  ...
  const netAfterRecorded=monthCollected-monthExpenses;
  ...
  return <>
    ...
    <section className="stats" style={{marginBottom:16}}>
      ...
      <Stat label="Monthly expenses" value={money(monthExpenses)} sub="Recorded expenses"/>
      <Stat label="Net collected" value={money(netAfterRecorded)} sub="Collected less recorded expenses"/>
    ...
    <section className="grid two">
      ...
      <div className="card">
        <div className="card-head"><div><h2>Maintenance</h2><p>{maint.filter(m=>m.status!=='Completed').length} open items</p></div></div>
      ...
      <div className="card">
        ...
        {docs.slice(0,6).map(d=><div className="reminder" key={d.id}><div><strong>{d.name}</strong><span>{d.category} · {d.documentDate||d.uploadedAt}</span><small>{d.fileName||''}</small></div></div>)}
