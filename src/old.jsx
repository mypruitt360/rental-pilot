...
export default function App(){
  ...
  const updateProperty = (id,property) => {
    ...
    const currentRent=Number(property.currentRent)||0;
    const potentialRent=Number(property.potentialRent)||currentRent;
    saveProperties(properties.map(p=>p.id===id?{...p,...property,capacity,occupied,currentRent,potentialRent,status:occupied>=capacity&&capacity>0?'Healthy':'Opportunity'}:p));
  };
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 0.9 · Property Controls</small><br/><button className="text-button" style={{marginTop:10}} onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
    </aside>
    <main>
      <header>
