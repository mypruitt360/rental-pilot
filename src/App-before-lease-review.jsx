...
export default function App(){
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 2.3.1 · Lease Review</small><br/><button className="text-button" style={{marginTop:10}} onClick={()=>supabase.auth.signOut()}>Sign out</button></div>
    </aside>
    <main>
      <header>
...
function CommandCenter({properties,residents,payments,expenses,maintenance,month,onRecord,onGo,onReviewLease}){
  ...
  const leaseAlerts=residents
    ...
    .map(r=>{
      ...
      return {...r,days,propertyName:property?.shortName||property?.name||'Unassigned',level};
    })
    .filter(r=>r.days<=90)
  ...
  return <>
    ...
    <section className="grid two">
      ...
      <div className="card">
        ...
        <div className="ledger">
          <div className="ledger-row ledger-head" style={{gridTemplateColumns:'1.25fr .8fr .75fr 110px',columnGap:14}}>
            <span>Resident</span><span>Property</span><span>Timing</span><span></span>
          </div>
          {leaseAlerts.map(r=><div className="ledger-row" key={r.id} style={{gridTemplateColumns:'1.25fr .8fr .75fr 110px',columnGap:14}}>
            <span style={{display:'flex',flexDirection:'column',gap:3,minWidth:0}}>
              <strong>{r.name}</strong>
              <small>Ends {r.leaseEnd}</small>
            </span>
            <span>{r.propertyName}</span>
            <span>
              <strong style={{color:r.days<0?'#b42318':r.days<=30?'#b42318':r.days<=60?'#b54708':'#667085'}}>
                {r.days<0?`${Math.abs(r.days)} days expired`:r.days===0?'Ends today':`${r.days} days`}
              </strong>
            </span>
...
function ResidentModal({mode='add',initial=null,properties,onClose,onSave}){
  const base={name:'',propertyId:properties[0]?.id||'',rent:'',email:'',phone:'',payerName:'',payerEmail:'',payerPhone:'',leaseStart:'',leaseEnd:'',dueDay:1};
