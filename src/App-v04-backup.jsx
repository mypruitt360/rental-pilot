...
import {
  ...
  CheckCircle2, Clock3, Target, TrendingUp, X, Save, ArrowRight, Sparkles
} from 'lucide-react';
...
const MAINTENANCE_KEY='rental-pilot-maintenance-v04';
...
export default function App(){
  ...
  const addProperty = property => {
    ...
    saveProperties([...properties,{...property,id,capacity,occupied,currentRent,potentialRent,status:occupied>=capacity&&capacity>0?'Healthy':'Opportunity'}]);
  };
  const addResident = resident => {
    const id=`resident-${Date.now()}`;
    const next={...resident,id,rent:Number(resident.rent)||0};
  ...
  const addMaintenance = item => saveMaintenance([{id:`maint-${Date.now()}`,status:'Open',createdAt:todayISO(),...item},...maintenance]);
  const updateMaintenance = (id,status) => saveMaintenance(maintenance.map(m=>m.id===id?{...m,status}:m));
  const removeMaintenance = id => saveMaintenance(maintenance.filter(m=>m.id!==id));

  const upsertPayment = payment => {
    const paymentMonth=payment.month||month;
    savePayments([...payments,{id:`pay-${Date.now()}`,...payment,month:paymentMonth}]);
  };
  ...
  return <div className="shell">
    <aside>
      ...
      <div className="aside-foot">Portfolio workspace<br/><small>Version 0.4 · Launch Build</small></div>
    </aside>
    <main>
      <header>
      ...
      {(section==='Command Center'||section==='Payments'||section==='Reminders') &&
        <MonthPicker month={month} setMonth={setMonth}/>}

      {section==='Command Center' && <CommandCenter properties={properties} residents={residents} payments={payments} month={month} onRecord={r=>setModal({type:'payment',residentId:r?.id})} onGo={setSection}/>}
      ...
      {section==='Properties' && <Properties properties={properties} residents={residents} payments={payments} month={month} onAdd={()=>setModal({type:'property'})}/>}
    ...
    {modal?.type==='property' && <PropertyModal onClose={()=>setModal(null)} onSave={p=>{addProperty(p);setModal(null);setSection('Properties');}}/>}
    ...
    {modal?.type==='maintenance' && <MaintenanceModal properties={properties} onClose={()=>setModal(null)} onSave={m=>{addMaintenance(m);setModal(null);}}/>}
  </div>
}
...
function CommandCenter({properties,residents,payments,month,onRecord,onGo}){
  ...
  return <>
    ...
    <section className="stats">
      ...
      <Stat label="Payments recorded" value={payments.filter(p=>p.month===month).length} sub={`${monthLabel(month)} entries`}/>
    </section>
...
function Properties({properties,residents,payments,month,onAdd}){
  return <>
    ...
    <section className="property-grid">{properties.map(p=>{
      ...
      return <div className="property-card" key={p.id}>
...
function Maintenance({maintenance,properties,onAdd,onStatus,onDelete}){
  ...
  return <div className="card">
    <div className="card-head"><div><h2>Maintenance</h2><p>Track requests from report to completion.</p></div><button className="primary" onClick={onAdd}><Plus size={16}/> Add Request</button></div>
    <div className="ledger">
      <div className="ledger-row ledger-head"><span>Property</span><span>Issue</span><span>Status</span><span>Date</span><span></span></div>
      {maintenance.map(m=><div className="ledger-row" key={m.id}>
        <span>{properties.find(p=>p.id===m.propertyId)?.shortName||'Property'}</span>
        <span><strong>{m.title}</strong><small>{m.notes}</small></span>
    ...
    {!!open.length&&<p style={{marginTop:16}}><strong>{open.length}</strong> open maintenance item{open.length===1?'':'s'}.</p>}
  </div>
}
...
function MaintenanceModal({properties,onClose,onSave}){
  const [form,setForm]=useState({propertyId:properties[0]?.id||'',title:'',notes:''});
  const set=(key,value)=>setForm(x=>({...x,[key]:value}));
  const save=()=>{
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" onMouseDown={e=>e.stopPropagation()}>
    <div className="modal-head"><div><h2>Add Maintenance Request</h2><p>Capture it now so it does not get lost.</p></div><button className="icon-button" onClick={onClose}><X/></button></div>
    <label>Property<select value={form.propertyId} onChange={e=>set('propertyId',e.target.value)}>{properties.map(p=><option value={p.id} key={p.id}>{p.shortName||p.name}</option>)}</select></label>
...
function PropertyModal({onClose,onSave}){
  ...
  const set=(key,value)=>setForm(x=>({...x,[key]:value}));
  const save=()=>{
    if(!form.name.trim()) return alert('Please enter the property name or address.');
    onSave({
      ...form,
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" onMouseDown={e=>e.stopPropagation()}>
    <div className="modal-head"><div><h2>Add Property</h2><p>Add the numbers you know now. You can refine them later.</p></div><button className="icon-button" onClick={onClose}><X/></button></div>
    <div className="form-grid">
      ...
      <label>Bathrooms<input type="number" value={form.bathrooms} onChange={e=>set('bathrooms',e.target.value)}/></label>
    ...
    <button className="primary wide save" onClick={save}><Save size={17}/> Save Property</button>
  </div></div>
}
